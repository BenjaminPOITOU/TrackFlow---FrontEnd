...
export default function AudioPlayer({
  ...
  return (
    <div className="w-full mx-auto">
      <div
        className={`bg-zinc-800 rounded-2xl shadow-lg border border-zinc-700 p-6 transition-opacity duration-300 ${isPlayerDisabled ? "opacity-60" : ""}`}
      >
        <div className="flex items-start gap-4 mb-6">
          <div className="flex-shrink-0 mt-1">
            <div
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                isPlaying && !isPlayerDisabled
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-lg font-semibold text-gray-100 truncate mb-1">
              VERSION :{" "}
              {isPlayerDisabled ? "Aucune" : versionNumber || "Sans titre"}
            </h2>
            <p className="text-sm text-zinc-400 truncate">
              BRANCHE :{" "}
              {isPlayerDisabled
                ? "Aucune sélectionnée"
                : branchName || "Branche inconnue"}
            </p>
          </div>
        </div>
        <div className="mb-4">
        ...
        <div className="flex justify-between text-sm text-zinc-400 mb-6">
          <span className="font-mono">
            {isPlayerDisabled ? "--:--" : formatTime(currentTime)}
          </span>
          <span className="font-mono">
            {isPlayerDisabled ? "--:--" : formatTime(duration)}
          </span>
        </div>
        <div className="flex items-center justify-between">
          <Button
            ...
            className={`w-12 h-12 rounded-full bg-gray-300 text-zinc-800 border-0 shadow-md transition-all duration-200 ${
              isPlayerDisabled
                ? "cursor-not-allowed"
                : "hover:bg-gray-400 hover:shadow-lg hover:scale-105 active:scale-95"
            }`}
          >
            {isPlaying ? (
          ...
          <div
            className={`flex items-center gap-3 w-32 transition-opacity duration-300 ${isPlayerDisabled ? "opacity-50" : ""}`}
          >
            <Button
              onClick={toggleMute}
              disabled={isPlayerDisabled}
