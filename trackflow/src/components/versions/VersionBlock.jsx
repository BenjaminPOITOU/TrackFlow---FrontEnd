...
export function VersionBlock({
  ...
  const fetchVersionDetails = useCallback(
    async (versionId) => {
      if (!versionId) return;
      ...
      try {
        ...
        if (!versionSummary || !versionSummary.branchId) {
          throw new Error(
            "Branch information is missing for the selected version."
          );
        }
        const versionDetails = await getVersionById({
          projectId,
          compositionId,
          branchId: versionSummary.branchId,
          versionId,
        });
    ...
    async (versionId) => {
      ...
      try {
        await deleteVersion({
          projectId,
          compositionId,
          branchId: currentVersion.branch.id,
          versionId,
        });
      } catch (err) {
  ...
  if (!detailsToDisplay && displayBranchList && displayBranchList.length > 0) {
    ...
    detailsToDisplay = {
      ...
      branchDescription: defaultBranch.description,
  ...
  return (
    <div className="relative flex w-full flex-col gap-3">
      ...
      <VersionBlockMediaPlayer
        key={detailsToDisplay?.id || "no-version"}
        versionId={detailsToDisplay?.id}
        versionName={detailsToDisplay?.name}
        branchName={detailsToDisplay?.branchName}
