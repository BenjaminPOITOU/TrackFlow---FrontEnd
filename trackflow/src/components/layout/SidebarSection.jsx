...
export default function SidebarSection({
  ...
  return (
    <div className="mb-4">
      <div
        className={`px-2 py-1.5 text-xs text-gray-400 uppercase tracking-wider flex items-center ${isCollapsed ? "justify-center" : "lg:justify-between"}`}
      >
        <span className={`hidden ${!isCollapsed && "lg:inline"}`}>{title}</span>
        <div
          className={`transition-opacity duration-300 opacity-0 ${isActiveSection && !isCollapsed && "lg:opacity-100"}`}
        ></div>
      </div>
